...
TEST_F(DotSearchSpaceTest, GenerateCandidateConfigsFiltersConfigsByHints) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
  ...
  std::vector<TritonGemmConfig> candidate_configs =
      search_space.GenerateCandidateConfigs({hint});
  EXPECT_THAT(candidate_configs, ElementsAre(hint));
}

TEST_F(DotSearchSpaceTest,
       GenerateCandidateConfigsWithEmptyDefaultConfigsReturnsAllConfigs) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
  std::vector<TritonGemmConfig> all_configs = search_space.GenerateConfigs();
  ...
  std::vector<TritonGemmConfig> candidate_configs =
      search_space.GenerateCandidateConfigs({});
  EXPECT_THAT(candidate_configs, ElementsAreArray(all_configs));
}

TEST_F(DotSearchSpaceTest,
       GenerateCandidateConfigsWithNonMatchingHintsFallsBackToHints) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
  ...
  std::vector<TritonGemmConfig> candidate_configs =
      search_space.GenerateCandidateConfigs({non_matching_hint});
  EXPECT_THAT(candidate_configs, ElementsAre(non_matching_hint));
}

TEST_F(DotSearchSpaceTest,
       GenerateCandidateConfigsWithWarpSpecializationPassesFlag) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       GetDefaultDotModule());
  TritonDotFusionSearchSpace search_space = MakeSearchSpace(module.get());
