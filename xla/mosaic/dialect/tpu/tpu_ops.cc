...
LogicalResult verifyCommonDmaParams(
  ...
  if (is_sc) {
    if (source_ty.getMemorySpace() == nullptr ||
        target_ty.getMemorySpace() == nullptr) {
      return op->emitOpError(
          "Source and target memory spaces must be provided for SparseCore "
          "DMA");
    }
    ...
    if (source_mem_space != nullptr && target_mem_space != nullptr) {
      if (failed(verifySparseCoreDmaSemaphores(
              op, source_mem_space, target_mem_space, source_sem, target_sem,
              is_remote, issuing_core))) {
        return failure();
      }
    }
  } else {
    if (target_sem == nullptr) {
      return op->emitOpError("DMA target semaphore must be specified");
    }
    if (!is_remote && source_sem != nullptr) {
      return op->emitOpError(
