...
TEST_P(ConcreteShardingSpecTest, UniqueIndexDomains) {
  ...
  ASSERT_OK_AND_ASSIGN(UniqueIndexDomains unique_index_domains,
                       sharding->UniqueIndexDomains(Shape({30})));
  EXPECT_THAT(unique_index_domains.index_domains,
              ElementsAre(IndexDomain(Index({0}), Shape({10})),
                          IndexDomain(Index({10}), Shape({20}))));
  ASSERT_NE(unique_index_domains.unique_shard_indices, nullptr);
  EXPECT_THAT(unique_index_domains.unique_shard_indices->shard_indices,
...
TEST_P(ConcreteEvenShardingSpecTest, UniqueIndexDomains) {
  {
    ...
    ASSERT_OK_AND_ASSIGN(UniqueIndexDomains unique_index_domains,
                         sharding->UniqueIndexDomains(Shape({30})));
    EXPECT_THAT(unique_index_domains.index_domains,
                ElementsAre(IndexDomain(Shape({30}))));
    ASSERT_NE(unique_index_domains.unique_shard_indices, nullptr);
    EXPECT_THAT(unique_index_domains.unique_shard_indices->shard_indices,
...
TEST_P(ShardingParamShardingSpecTest, UniqueIndexDomains) {
  {
    ...
    ASSERT_OK_AND_ASSIGN(UniqueIndexDomains unique_index_domains,
                         sharding->UniqueIndexDomains(shape));
    EXPECT_THAT(unique_index_domains.index_domains,
                ElementsAre(IndexDomain(Index({0, 0}), Shape({2, 3})),
                            IndexDomain(Index({0, 3}), Shape({2, 3})),
                            IndexDomain(Index({0, 6}), Shape({2, 3})),
                            IndexDomain(Index({2, 0}), Shape({2, 3})),
                            IndexDomain(Index({2, 3}), Shape({2, 3})),
                            IndexDomain(Index({2, 6}), Shape({2, 3}))));
    ASSERT_NE(unique_index_domains.unique_shard_indices, nullptr);
    EXPECT_THAT(unique_index_domains.unique_shard_indices->shard_indices,
  ...
  }
  {
    ...
    ASSERT_OK_AND_ASSIGN(UniqueIndexDomains unique_index_domains,
                         sharding->UniqueIndexDomains(shape));
    EXPECT_THAT(unique_index_domains.index_domains,
                ElementsAre(IndexDomain(Index({0, 0}), Shape({2, 9})),
                            IndexDomain(Index({2, 0}), Shape({2, 9}))));
    ASSERT_NE(unique_index_domains.unique_shard_indices, nullptr);
    EXPECT_THAT(unique_index_domains.unique_shard_indices->shard_indices,
