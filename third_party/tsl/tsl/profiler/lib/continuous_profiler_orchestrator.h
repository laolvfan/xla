...
class ContinuousProfilerOrchestrator : public ProfilerInterface {
 public:
  ...
  DrainedBuffer PopBufferWithTelemetry() {
    std::deque<ConsumeResult> local_buffer;
    ...
    for (auto& item : local_buffer) {
      if (item.data.has_value()) {
        chunks.push_back(std::move(item.data));
      }
    }
 ...
 private:
  void PushChunkLocked(ConsumeResult chunk)
    ...
    if (!chunk.data.has_value() || chunk.estimated_size_bytes == 0) {
      return;
    }
    ...
    while (!circular_buffer_.empty() &&
           (total_buffered_bytes_ + chunk.estimated_size_bytes >
            max_buffer_bytes_)) {
      size_t front_size = circular_buffer_.front().estimated_size_bytes;
    ...
    total_buffered_bytes_ += chunk.estimated_size_bytes;
    circular_buffer_.push_back(std::move(chunk));
  }
  ...
  std::deque<ConsumeResult> circular_buffer_ ABSL_GUARDED_BY(mutex_);
